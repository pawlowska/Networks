...
mymat <- lista.corr[["relapse"]]

lookup<-unique(dane[,c("abbrev", "category")])

mydf <- data.frame(category=lookup$category)
row.names(mydf)<-lookup$abbrev

kolejnosc<-colnames(mymat)
kolejnosc<-kolejnosc[order(match(kolejnosc, lookup$abbrev))]
#print(kolejnosc[1:20])

mymat<-mymat[kolejnosc, kolejnosc]

# add row annotations
my_heatmap<-pheatmap(mymat, cluster_cols = F, cluster_rows = F, annotation_row = mydf)
